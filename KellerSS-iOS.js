...
function analyzeIps(parsed) {
  ...
  for (let e of entries) {
    ...
    if (CLEAN_WHITELIST.some(w => {
      let lw = w.toLowerCase();
      return bid.toLowerCase() === lw || bid.toLowerCase().includes("." + lw);
    })) continue;
...
function classifyIP(info, domain) {
  ...
  let isWhitelisted = CLEAN_WHITELIST.some(w => {
    let lw = w.toLowerCase();
    return (domLow === lw || domLow.endsWith("." + lw)) || (ipStr === lw);
  });
  if (isWhitelisted) {
...
async function analyze(entries) {
  ...
  let netEntries = entries.filter(e => {
    ...
    if (CLEAN_WHITELIST.some(w => {
      let lw = w.toLowerCase();
      return domain === lw || domain.endsWith("." + lw);
    })) return false;
  ...
  for (let i = 0; i < allDomains.length; i += CHUNK) {
    ...
    for (let j = 0; j < results.length; j++) {
      ...
      let isWhitelistedFinal = CLEAN_WHITELIST.some(w => {
        let lw = w.toLowerCase();
        return (domain.toLowerCase() === lw || domain.toLowerCase().endsWith("." + lw)) || (ip.toLowerCase() === lw);
      });
  ...
  for (let e of netEntries) {
    ...
    if (CLEAN_WHITELIST.some(w => {
      let lw = w.toLowerCase();
      return (dom === lw || dom.endsWith("." + lw)) || (bid.toLowerCase() === lw || bid.toLowerCase().includes("." + lw));
    })) continue;
...
function buildHTML(findings, netEntries, cheatAppFindings, knownCheatFindings, ipsFindings, ipsMeta, _unused, ghostAppFindings, proxyLoginFindings, filename) {
  ...
  function isClean(text) {
    ...
    return FINAL_WHITELIST.some(w => {
      let lw = w.toLowerCase();
      return t === lw || t.endsWith("." + lw) || t === lw || t.includes("." + lw);
    });
  }
