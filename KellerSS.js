...
// ─── Firebase Config ──────────────────────────────────────────────────────────
const FIREBASE_DB_URL  = "https://kellerfuck-default-rtdb.firebaseio.com"
const FIREBASE_API_KEY = "AIzaSyCj-1Tv7bdmUirQ2hgCb9w-nLHVgfRQS3s"

// ─── Verificar se o IP está na whitelist do Firebase ─────────────────────────
async function checkWhitelist() {
  try {
    let ipReq = new Request("https://api.ipify.org?format=json")
    ipReq.timeoutInterval = 8
    let ipData = await ipReq.loadJSON()
    let myIP = (ipData && ipData.ip) ? ipData.ip : null
    if (!myIP) return false
    let key = myIP.replace(/\./g, "_").replace(/:/g, "_")
    let req = new Request(FIREBASE_DB_URL + "/whitelist/" + key + ".json?auth=" + FIREBASE_API_KEY)
    req.method = "GET"
    req.timeoutInterval = 10
    let data = await req.loadJSON()
    if (data && data.status === "active") {
      // Verifica se o bypass já expirou
      if (data.expires_at && new Date() > new Date(data.expires_at)) {
        // Se expirou, remove da whitelist automaticamente
        let delReq = new Request(FIREBASE_DB_URL + "/whitelist/" + key + ".json?auth=" + FIREBASE_API_KEY)
...
async function main() {
  ...
  // ─── Verificar whitelist Firebase ────────────────────────────────────────
  let isWhitelisted = await checkWhitelist()

  if (isWhitelisted) {
    // IP na whitelist: passa arrays vazios para buildHTML → contadores ficam 0/0/0
    // O design, textos e layout do resultado são 100% os originais do KellerSS
    let html = buildHTML([], entries.filter(e => e.type === "networkActivity"), [], [], [], ipsMeta, [], [], [], filename)
    await showResult(html)
    return
  }

  // IP fora da whitelist: análise real completa
